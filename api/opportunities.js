  ...
  try {
    ...
    if (event.httpMethod === 'POST') {
      ...
      const [opp] = await sql`INSERT INTO opportunities (
        ...
        closed_reason, closed_note, deal_notes, salesforce_url, folder_url, deal_registration_url, user_id
      ) VALUES (
        ${b.title},
        ${toInt(b.contact_id)},
        ...
        ${toNull(b.folder_url) || ''},
        ${toNull(b.deal_registration_url) || ''},
        ${user.id}
      ) RETURNING *`;
    ...
    if (event.httpMethod === 'PUT') {
      ...
      const [opp] = await sql`UPDATE opportunities SET
        ...
        value=CASE WHEN ${b.value !== undefined && b.value !== '' ? 1 : 0}=1 THEN ${b.value !== undefined && b.value !== '' ? Math.round(Number(b.value)) : null} ELSE value END,
        probability=CASE WHEN ${b.probability !== undefined && b.probability !== '' ? 1 : 0}=1 THEN ${b.probability !== undefined && b.probability !== '' ? Math.round(Number(b.probability)) : null} ELSE probability END,
        ...
        deal_registration_url=COALESCE(${b.deal_registration_url !== undefined ? (toNull(b.deal_registration_url) || '') : null},deal_registration_url),
