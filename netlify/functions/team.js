...
// GET              — Get user's teams and members
// POST { name }    — Create team
// POST { action: "join", code } — Join by invite code
// POST { action: "members", teamId } — Get team members
// ═══════════════════════════════════════════════════════
      ...
      const teams = await sql`
        SELECT t.*, tm.role,
          (SELECT json_agg(jsonb_build_object(
            'id', u.id, 'name', u.name, 'initials', u.initials, 'color', u.color, 'role', tm2.role
          ))
